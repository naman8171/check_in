...
class LoanPortalController(http.Controller):
    ...
    @http.route('/loans/apply', type='http', auth='user', website=True)
    ...
    @http.route('/loans/apply/submit', type='http', auth='user', website=True, methods=['POST'], csrf=True)
    def loan_apply_submit(self, **post):
        partner = request.env.user.partner_id
        loan_type_id = int(post.get('loan_type_id'))
        ...
        loan = request.env['loan.loan'].sudo().create({
